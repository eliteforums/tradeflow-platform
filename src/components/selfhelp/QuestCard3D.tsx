...
/* ── Lightweight 2D card grid (used on mobile & as fallback) ── */
...
/* ── 3D version (desktop only, lazy loaded) ── */
let Canvas3D: React.ComponentType<QuestCard3DProps> | null = null;

try {
  // Dynamic import so mobile never downloads Three.js bundles for this
  const { Canvas } = require("@react-three/fiber");
  const { Text, RoundedBox } = require("@react-three/drei");
  ...
  function QuestCardMesh({
    ...
    useFrame((state: any) => {
      ...
      ref.current.position.y =
        position[1] + Math.sin(t * 1.2 + position[0] * 2) * 0.05;
  ...
  Canvas3D = function DesktopQuestCards({
    quests,
    completedIds,
    onComplete,
  }: QuestCard3DProps) {
...
export default function QuestCard3D(props: QuestCard3DProps) {
  ...
  // Always use 2D on mobile for performance/reliability
  if (isMobile || !Canvas3D) {
    return <QuestCards2D {...props} />;
  }
