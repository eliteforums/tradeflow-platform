...
import MobileSelfHelp from "@/components/mobile/MobileSelfHelp";
...
const SelfHelp = () => {
  ...
  const startBreathing = () => { setBreathCount(0); setBreathPhase("inhale"); };
  ...
  if (isMobile) return <MobileSelfHelp />;
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-4">
          ...
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center justify-center gap-1.5 py-2 px-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id ? `bg-gradient-to-br ${tab.gradient} text-white shadow-sm` : "text-muted-foreground hover:text-foreground"
              }`}>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "quest" && (
            <motion.div key="quest" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="space-y-3">
              <QuestCard3D quests={quests.map((q) => ({ id: q.id, title: q.title, xp_reward: q.xp_reward }))} completedIds={completedIds}
                onComplete={(id) => { const quest = quests.find((q) => q.id === id); if (quest && !getQuestStatus(quest.id)) completeQuest(quest); }} />
              <div className="p-4 rounded-xl bg-card border border-border/50">
                ...
                <p className="text-xs text-muted-foreground">{completedToday === quests.length ? "🎉 All quests completed!" : "Complete all quests to earn bonus XP!"}</p>
              </div>
              <div className="space-y-2">
                {quests.map((quest) => {
                  const isCompleted = getQuestStatus(quest.id);
                  return (
                    <div key={quest.id} className={`p-3 rounded-xl border transition-all ${isCompleted ? "bg-emerald-500/5 border-emerald-500/20" : "bg-card border-border/50"}`}>
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isCompleted ? "bg-emerald-500/15" : "bg-gradient-to-br from-amber-500 to-orange-500"}`}>
                          {isCompleted ? <CheckCircle className="w-4 h-4 text-emerald-500" /> : <Award className="w-4 h-4 text-white" />}
                        </div>
                        ...
                        {!isCompleted && (
                          <Button size="sm" className="h-7 px-2.5 text-xs shrink-0" onClick={() => completeQuest(quest)} disabled={isCompleting}>
                            {isCompleting ? <Loader2 className="w-3 h-3 animate-spin" /> : <>Do it<ChevronRight className="w-3 h-3 ml-0.5" /></>}
                          </Button>
                        )}
                      </div>
                    </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {achievements.map((a) => (
                    <div key={a.id} className={`p-3 rounded-xl text-center border ${a.unlocked ? "bg-card border-primary/30" : "bg-muted/20 border-border/50 opacity-40"}`}>
                      <a.icon className={`w-6 h-6 mx-auto mb-1 ${a.unlocked ? "text-primary" : "text-muted-foreground"}`} />
          ...
          {activeTab === "wreck" && (
            <motion.div key="wreck" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="space-y-3">
              ...
              <div className="p-5 rounded-xl bg-card border border-border/50 text-center">
                <h2 className="text-xl font-bold font-display mb-1">Wreck the Buddy</h2>
                <p className="text-xs text-muted-foreground mb-3">Tap to release stress. 30 hits = 1 ECC!</p>
                ...
                <Button className="bg-gradient-to-br from-red-500 to-pink-500 text-white h-11 px-5" onClick={handleWreckClick}>
          ...
          {activeTab === "tibetan" && (
            <motion.div key="tibetan" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="space-y-3">
              <TibetanBowl3D phase={breathPhase} />
              <div className="p-5 rounded-xl bg-card border border-border/50 text-center">
                <h2 className="text-xl font-bold font-display mb-1">Tibetan Bowl</h2>
                <AnimatePresence mode="wait">
                  {breathPhase === "idle" ? (
                    <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <p className="text-xs text-muted-foreground mb-4 max-w-xs mx-auto">4-7-8 breathing with Tibetan singing bowls.</p>
                      <Button className="bg-gradient-to-br from-violet-500 to-purple-500 text-white h-11 px-5" onClick={startBreathing}>
                    ...
                    <motion.div key={breathPhase} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="py-3">
                      <p className="text-2xl font-bold font-display text-primary mb-1">
                        {breathPhase === "inhale" && "Breathe In..."}{breathPhase === "hold" && "Hold..."}{breathPhase === "exhale" && "Breathe Out..."}
                      </p>
                      <p className="text-xs text-muted-foreground">{breathPhase === "inhale" && "4 seconds"}{breathPhase === "hold" && "7 seconds"}{breathPhase === "exhale" && "8 seconds"}</p>
                      <p className="text-[10px] text-muted-foreground mt-2.5">Cycle {breathCount + 1} of 3</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
