    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold font-display mb-1">Expert Appointments</h1>
            <p className="text-sm text-muted-foreground">Book sessions with verified professionals</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-card border border-border self-start">
            <Coins className="w-4 h-4 text-primary" />
            <span className="font-semibold text-sm">{balance} ECC</span>
          </div>
        </div>

        {/* My Appointments */}
        <div className="bg-card border border-border rounded-2xl p-4 sm:p-6">
          <h2 className="text-base font-semibold font-display mb-3 sm:mb-4">My Appointments</h2>
          <div className="flex gap-2 mb-4 sm:mb-6">
            <Button variant={activeTab === "upcoming" ? "default" : "ghost"} onClick={() => setActiveTab("upcoming")} size="sm" className={activeTab === "upcoming" ? "bg-primary text-primary-foreground" : ""}>
              Upcoming ({upcomingAppointments.length})
            </Button>
            <Button variant={activeTab === "past" ? "default" : "ghost"} onClick={() => setActiveTab("past")} size="sm" className={activeTab === "past" ? "bg-primary text-primary-foreground" : ""}>
              Past
            </Button>
          </div>
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {(activeTab === "upcoming" ? upcomingAppointments : pastAppointments).length === 0 ? (
              ...
              (activeTab === "upcoming" ? upcomingAppointments : pastAppointments).map((apt) => (
                <div key={apt.id} className="p-3 sm:p-4 rounded-xl bg-muted/30 border border-border">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          {apt.session_type === "video" ? <Video className="w-3 h-3" /> : <Phone className="w-3 h-3" />}
                          {apt.session_type}
                        </span>
                      </div>
                      {activeTab === "upcoming" && (apt.status === "confirmed" || apt.status === "pending") && (
                        <div className="flex items-center gap-2 mt-2">
                          <Button variant="outline" size="sm" onClick={() => handleJoinCall(apt.session_type)} className="gap-1 h-7 text-xs">
                            {apt.session_type === "video" ? <Video className="w-3 h-3" /> : <Phone className="w-3 h-3" />} Join
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => cancelAppointment(apt.id)} className="text-destructive h-7 text-xs">
        ...
        <div>
          <div className="flex items-center justify-between mb-3 sm:mb-4 gap-3">
            ...
            <div className="relative hidden sm:block">
              ...
              <Input placeholder="Search experts..." className="pl-9 w-48 bg-card h-9 text-sm" />
            </div>
          </div>
          ...
          {experts.length === 0 ? (
            <div className="text-center py-10 text-muted-foreground bg-card rounded-2xl border border-border">
              ...
              <p className="text-sm">No experts available at the moment</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {experts.map((expert) => {
                ...
                return (
                  <div key={expert.id} className="p-4 sm:p-5 rounded-2xl bg-card border border-border hover:border-primary/50 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center">
                        <User className="w-5 h-5 sm:w-7 sm:h-7 text-white" />
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[11px] ${hasSlots ? "bg-eternia-success/10 text-eternia-success" : "bg-muted text-muted-foreground"}`}>
                        {hasSlots ? "Available" : "No slots"}
                      </span>
                    </div>
                    <h3 className="font-semibold text-sm sm:text-base mb-0.5">{expert.username}</h3>
                    <p className="text-xs sm:text-sm text-muted-foreground mb-2">{expert.specialty || "Mental Health Professional"}</p>
                      ...
                      <div className="space-y-1.5 mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {expertSlots.slice(0, 3).map((slot) => (
                            <Button key={slot.id} variant="outline" size="sm" onClick={() => handleBookSlot(expert, slot)} className="text-[11px] h-7 px-2">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border">
                      <div>
                        ...
                        <p className="font-semibold text-sm flex items-center gap-1"><Coins className="w-3.5 h-3.5 text-primary" /> 50 ECC</p>
                      </div>
                      {hasSlots && (
                        <Button size="sm" className="bg-primary text-primary-foreground h-8 text-xs" onClick={() => handleBookSlot(expert, expertSlots[0])}>
                          Book Now
                        </Button>
                      )}
                    </div>
      ...
      <Dialog open={bookingDialog.open} onOpenChange={(open) => setBookingDialog({ open })}>
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Confirm your appointment with {bookingDialog.expert?.username}</DialogDescription>
          </DialogHeader>
          {bookingDialog.slot && (
            <div className="space-y-4 py-2">
              <div className="p-3 sm:p-4 rounded-lg bg-muted/30 border border-border space-y-2">
                <div className="flex items-center gap-3">
                  <Calendar className="w-4 h-4 text-primary shrink-0" />
                  <span className="text-sm font-medium">{format(new Date(bookingDialog.slot.start_time), "EEEE, MMMM d, yyyy")}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Clock className="w-4 h-4 text-primary shrink-0" />
                  <span className="text-sm">{format(new Date(bookingDialog.slot.start_time), "h:mm a")} - {format(new Date(bookingDialog.slot.end_time), "h:mm a")}</span>
                </div>
              </div>
              <div>
              ...
              {balance < 50 && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-xs">Insufficient credits.</div>
              )}
            </div>
          )}
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setBookingDialog({ open: false })} size="sm">Cancel</Button>
            <Button onClick={confirmBooking} disabled={isBooking || balance < 50} size="sm" className="btn-primary gap-1.5">
