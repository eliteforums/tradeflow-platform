...
Format the report professionally with clear headers and bullet points.`,
    ...
    const report = `# 📊 Eternia Analytics Report
**Generated:** ${now.toLocaleString("en-IN", { dateStyle: "long", timeStyle: "short" })}  
**Period:** ${rangeDate.toISOString().split("T")[0]} to ${now.toISOString().split("T")[0]} (${dateRange})  
**Report Type:** Comprehensive Site Analytics
...
| Views Today | ${viewsToday.toLocaleString()} |
| Views This Week | ${viewsWeek.toLocaleString()} |
| Unique Visitors | ${uniqueVisitors.toLocaleString()} |
| Authenticated Users | ${authUsers.size.toLocaleString()} |
| Anonymous Visitors | ${anonSessions.size.toLocaleString()} |
| Total Sessions | ${totalSessions.toLocaleString()} |
| Bounce Rate | ${bounceRate}% |
| Pages/Session | ${pagesPerSession} |
| Registered Users | ${totalUsers?.toLocaleString() || "N/A"} |

### Device Breakdown
| Device | Views | % |
|--------|-------|---|
| Desktop | ${devices.desktop.toLocaleString()} | ${totalViews > 0 ? Math.round((devices.desktop / totalViews) * 100) : 0}% |
| Mobile | ${devices.mobile.toLocaleString()} | ${totalViews > 0 ? Math.round((devices.mobile / totalViews) * 100) : 0}% |
| Tablet | ${devices.tablet.toLocaleString()} | ${totalViews > 0 ? Math.round((devices.tablet / totalViews) * 100) : 0}% |

### Browser Usage
| Browser | Views | % |
|---------|-------|---|
${Object.entries(browserCounts).sort(([, a], [, b]) => b - a).map(([b, c]) => `| ${b} | ${c.toLocaleString()} | ${Math.round((c / totalViews) * 100)}% |`).join("\n")}

### Top Pages
| # | Page | Views |
|---|------|-------|
${topPages.map((p, i) => `| ${i + 1} | ${p.path} | ${p.count.toLocaleString()} |`).join("\n")}

### Traffic Sources
| # | Source | Visits |
|---|--------|--------|
${topReferrers.map((r, i) => `| ${i + 1} | ${r.source} | ${r.count.toLocaleString()} |`).join("\n")}

### Daily Traffic Trend
| Date | Views |
|------|-------|
${dailyTrend.map(([d, c]) => `| ${d} | ${(c as number).toLocaleString()} |`).join("\n")}
...
---
*Report generated by Eternia Analytics Engine*
`;

    return new Response(JSON.stringify({ report, dateRange }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
